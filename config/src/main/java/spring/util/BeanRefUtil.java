...
//				String fieldKeyName = parKeyName(field.getName());
     ...
     * @param user
     */
    public static void setInsertDefaultFieldValue(Object bean, Map<String, String> valMap, SysUser user) {
        valMap.put("createBy", user == null ? "" : user.getId());
        valMap.put("createDate", BeanRefUtil.fmtDate(new Date()));
        valMap.put("updateDate", BeanRefUtil.fmtDate(new Date()));
        valMap.put("updateBy", user == null ? "" : user.getId());
        valMap.put("id", IdWorker.getIdWorkerNext().toString());
        valMap.put("parent", "0");
        BeanRefUtil.setFieldValue(bean, valMap);
    }
